 ...
 */
public class DashboardWebViewManager extends JavaFxBrowserManager {
	...
	private String buildCreateWizards() {
		...
		for (int i = 0; i < ids.length; i++) {
			IConfigurationElement element = BrowserUtils.getExtension(EXTENSION_ID_NEW_WIZARD, ids[i]);
			if (element != null && element.getAttribute(BrowserUtils.ELEMENT_CLASS) != null
					&& element.getAttribute(BrowserUtils.ELEMENT_NAME) != null
					&& element.getAttribute(BrowserUtils.ELEMENT_ICON) != null) {
				...
				html += "<a class=\"ide_widget btn btn-black uppercase\" href=\"\" onclick=\"ide.call('openWizard', '"
						...
						+ "')\">"
						+ "Create "
						+ element.getAttribute(BrowserUtils.ELEMENT_NAME) + "</a>";
			}
		}
		return html;
	}
	...
	private String buildUpdate(UpdateNotification notification) {
		...
		html += "	<a href=\"\" onclick=\"ide.call('openWebPage', '" + entry.getLink() + "')\"><b>"
				+ entry.getTitle() + "</b></a>";
		html += "	</div>";
		html += "</div>";
		return html;
